...
- **Tailored presets (replaced the old live AI Tailor feature on 2026-07-26)**: `js/resume-presets.js` holds a hand-written `resumePresets` object, keyed by slug (currently just `ag-engineering`). Entirely static — no API calls, no key required from visitors. The old version called xAI's Grok API directly from the browser using a visitor-supplied key stored in `localStorage`; that only ever worked for James himself (recruiters won't have an xAI key) and was removed. New presets are added periodically by hand in `js/resume-presets.js` — every bullet in a preset must be copied/reworded from `resumeData`, never invented, per the comment at the top of that file.
  - **UI**: preset buttons are injected as plain `.focus-btn` elements (via `#tailor-selector`, a `display:contents` span inside `.focus-selector`) — same row, same CSS, same mutual-exclusivity as All Skills/Robotics/etc. There is deliberately no separate "General"/dropdown/toggle control and no "Tailored for…" banner on the resume itself; clicking a preset button behaves exactly like clicking any other focus button (`resume-generator.js`'s single `focusButtons.forEach(...)` click handler branches on `this.dataset.preset` vs `this.dataset.focus`). Because of this, `focusButtons` **must** be queried only after the preset buttons are appended to the DOM, or new presets silently won't be clickable.
  - `applyPreset()` renders a preset's curated `subtitle`/`skillsSummary`/bullet selections in place of the default `resumeData` view; clicking a plain focus button clears `activePreset` and returns to normal rendering.
